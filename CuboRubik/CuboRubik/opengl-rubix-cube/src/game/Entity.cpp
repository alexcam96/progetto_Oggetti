...
bool Entity::operator==(const Entity& right) const {
	if (children_ == right.children_) {
		if (def_transform_ == right.def_transform_ and def_transform_i_ == right.def_transform_i_) {
			if (n_ == right.n_ and o_ == right.o_ and u_ == right.u_ and v_ == right.v_) {
				if (parent_ == right.parent_) {
					if (transform_ == right.transform_ and transform_i_ == transform_i_) {
						return true;
					}
				}
			}
